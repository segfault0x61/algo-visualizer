...
export class BinarySearch {
  ...
  binarySearch(arr: ArrayBars[], target: number): number {
    ...
    while (left <= right) {
      ...
      if (arr[mid].value < target) {
        // Values is on the right side, ignore the left side
        this.animations.push({
          selectedIndex: null,
          disabledIndexs: this.getIndexRange(left, mid),
        });
        left = mid + 1;
      } else {
        this.animations.push({
          selectedIndex: null,
          disabledIndexs: this.getIndexRange(mid, right),
        });
        right = mid - 1;
      }
    }
  ...
  getIndexRange(left: number, right: number): number[] {
    ...
    for (let i = left; i <= right; i++) indexes.push(i);
    return indexes;
  }
  ...
  binarySearchAnimation(): void {
    ...
    const timer = setInterval(() => {
      ...
      if (action) {
        if (action.selectedIndex && this.animations.length === 0)
          // Last index, animation is finished
          this.arrService.numbers[action.selectedIndex].color =
            this.arrService.$finishedBars;
        else if (action.selectedIndex)
          this.arrService.numbers[action.selectedIndex].color =
            this.arrService.$selectedIndex;
        else
          action.disabledIndexs?.map(
            (num) => (this.arrService.numbers[num].color = '#DCDCDC')
          );
      } else {
        clearInterval(timer);
